...
-   Using any tool (except R functions that automate the process) compute the centered moving average for the chocolate data. To help your check yourself, the value of $\hat m$ in month 7 should be `r chocolate_month_ts %>% data.frame() %>% filter(row_number() == 7) %>% dplyr::select(m) %>% pull()`.
...
plain <- autoplot(chocolate_month_ts, .vars = m) +
  labs(
    x = "Month",
    y = "Searches",
...
fancy <- autoplot(chocolate_month_ts, .vars = value) +
  ...
  geom_line(aes(x = dates, y = m), color = "#D55E00") +
  theme(plot.title = element_text(hjust = 0.5))

plain | fancy
```

If you stored your centered moving average in a variable called "m" in the "chocolate_month_ts" tsibble, you can generate the plot on the right with the R command:
