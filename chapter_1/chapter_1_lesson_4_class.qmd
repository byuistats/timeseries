...
### Converting the data file to a tsibble
...
# Note that this file is not in the right order, so this gives the wrong tsibble
...
Next, we compute the centered moving average, $\hat m$:
...
temp <- deaths_ts |>
  dplyr::select(index, dates, year, month, value, m_hat) |>
  round_df(3)

for (i in 1:13) {
  temp$value[i] = cell_spec(temp$value[i], color = "#E69F00") 
}
temp$m_hat[7] = cell_spec(temp$m_hat[7], color = "#E69F00")
