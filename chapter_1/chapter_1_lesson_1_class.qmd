...
-   Think about what you know about the reported number of searches in December compared to the following February. The reported number of searches for "chocolate" in December `` `r knitr::inline_expr("example_year-1")` `` is `` `r knitr::inline_expr("chocolate_month_ts |> filter(year == example_year - 1, month == 12) |> data.frame() |> select(value) |> pull()")` ``. Does it make sense that the reported number of searches in February `` `r knitr::inline_expr("example_year")` `` is `` `r knitr::inline_expr("chocolate_month_ts |> filter(year == example_year, month == 2) |> data.frame() |> select(value) |> pull()")` ``? Given the value from December, is the value in the following February independent and completely random?
-   The value reported by Google for June `` `r knitr::inline_expr("example_year")` `` is `` `r knitr::inline_expr("chocolate_month_ts |> filter(year == example_year, month == 6) |> data.frame() |> select(value) |> pull()")` ``. Based on what you have observed in the data, do you think the value for July `` `r knitr::inline_expr("example_year")` `` will be close to or far from this value? Justify your answer.
:::
