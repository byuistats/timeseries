...
deaths_shat_ts |>
  ...
  mutate(
    s_hat = ifelse(row_number() <= 14, "______", s_hat),
...
-   Subtract the mean of the $\hat s_t$ values for each month from the mean of these means to get to get $\bar s_t$, the seasonally adjusted mean for month $t$. 
...
two_more_rows_from_adjusted_ts <- head(adjusted_ts,16) |> 
  tail(2) |> 
...
adjusted_ts |>
  ...
  convert_df_to_char() |>
  bind_rows(two_more_rows_from_adjusted_ts) |>
  display_table()
```
...
seasplot <- autoplot(adjusted_ts, .vars = seasonally_adjusted_x) +
  # geom_line(aes(x = index, y = seasonally_adjusted_x), color = "#D55E00") +
  # geom_line(aes(x = index, y = m_hat), color = "#0072B2") +
  labs(
    x = "Month",
