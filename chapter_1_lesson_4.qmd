...
## Converting from a Data File to a Tsibble (10 min)

This is a demonstration of how to convert an Excel 
...
# Use the date features in the data frame to construct a date-time object.
# Note: This file is not in the right order, but that doesn't matter because
# the months and years are given for each row.
# We first create a tibble, and then convert this to a tsibble.
# There are a lot of ways to accomplish the task of creating a tsibble.
# You can do whatever works best for you.

deaths_tibble <- deaths_df |>
  mutate(
    date_str = paste("1", Month, Year),  # Create a string of the form "1 mmm yyyy"
    dates = dmy(date_str),               # Convert the string to date-time object
    year = lubridate::year(dates),       # Extract the year (numeric)
    month = lubridate::month(dates),     # Extract the month (numeric)
    value = Deaths                       # Create a column of the observed data
  ) |>
...
deaths_ts <- deaths_tibble |>
  mutate(index = tsibble::yearmonth(dates)) |>      # Define the index variable
...
-   How do the positions of the box plots correspond to the times of the year where you noted more (or fewer) deaths occurring?
-   What could explain this observation?
-   Does this surprise you? Explain your reasoning.

:::

## Computing the Seasonally Adjusted Series (40 min)

Our objective is to find an estimate for the time series that does not fluctuate with the seasons. This is called the seasonally adjusted series. Download the Excel file 
<a href="handouts/chapter_1_4_handout.xlsx" download="chapter_1_4_handout.xlsx"> chapter_1_4_handout.xlsx </a>
and use it to organize your work.
...
$$
  \hat s_t = x_t - \hat m_t
$$
To emphasize the computation of the centered moving average, the observed data values that were used to find $\hat m_t$ for December 2017 are shown in <span style="color:#fd7e14;">orange</span> in the table below. 
...
deaths_shat_ts |>
  ...
  mutate(
    s_hat = ifelse(row_number() > num_sample_rows, "______", s_hat),
  ...
  color_specific_cell(row_num = 12, col_num = 3, color = "#fd7e14") |>
  color_specific_cell(row_num = 6, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 7, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 8, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 9, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 10, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 11, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 12, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 13, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 14, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 15, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 16, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 17, col_num = 2, color = "#fd7e14") |>
  color_specific_cell(row_num = 18, col_num = 2, color = "#fd7e14") |>
  display_table()
```
...
-   Working with your assigned partner, fill in the missing values of $\hat s_t$ in the corresponding table in the handout 
<a href="handouts/chapter_1_4_handout.xlsx" download="chapter_1_4_handout.xlsx"> chapter_1_4_handout.xlsx </a>. 
...
wider_df2 |>
  ...
  mutate(Apr = ifelse(row_number() == n(), "______", Apr)) |>
  mutate(May = ifelse(row_number() == n(), "______", May)) |>
  mutate(Jun = ifelse(row_number() == n(), "______", Jun)) |>
  mutate(Jul = ifelse(row_number() == n(), "______", Jul)) |>
  mutate(Aug = ifelse(row_number() == n(), "______", Aug)) |>
  mutate(Sep = ifelse(row_number() == n(), "______", Sep)) |>
  mutate(Oct = ifelse(row_number() == n(), "______", Oct)) |>
...
-   The table above gives the values of $\hat s_t$. Fill in the missing values in the first two rows of the corresponding table in the Excel file <a href="handouts/chapter_1_4_handout.xlsx" download="chapter_1_4_handout.xlsx"> chapter_1_4_handout.xlsx </a>. (Note you already computed these.)
...
-   Compute the mean of the $\bar {\hat s_t}$ values. Call this value $\bar {\bar {\hat s_t}}$ (This number should equal 0, except for any rounding.)
-   Subtract the grand mean $\bar {\bar {\hat s_t}}$ from the column means $\bar {\hat s_t}$ to get $\bar s_t$, the seasonally adjusted mean for month $t$.  (Note that the mean of the $\bar s_t$ values will be relatively close to zero, but not necessarily zero. If the number of years that you have data is not the same for all months, this values will not be zero.)

$$ \bar s_t = \bar {\hat s_t} - \bar {\bar {\hat s_t}} $$

Finally, $\bar s_t$ is the seasonally adjusted mean for month $t$. 
...
Fill in the missing values in the corresponding table in the Excel file <a href="handouts/chapter_1_4_handout.xlsx" download="chapter_1_4_handout.xlsx"> chapter_1_4_handout.xlsx </a>.
...
adjusted_ts |>
  head(num_sample_rows + num_addl_rows) |> 
  convert_df_to_char(1) |>
  mutate(
    s_hat = if_else(row_number() > num_sample_rows, "______", s_hat),
    s_bar = if_else(row_number() > 1, "______", s_bar),
    random = if_else(row_number() > 7, "______", random),
    seasonally_adjusted_x = if_else(row_number() > 1, "______", seasonally_adjusted_x)
  ) |>
  ...
  rename(
    ...
    "$$ \\bar s $$" = s_bar,
    Random = random,
    "Seasonally Adjusted Series" = seasonally_adjusted_x
  ) |>
  display_partial_table(num_sample_rows + num_addl_rows, 0)
```



## Class Activity: Computing the Additive Decomposition in R (15 min)
...
<a href="handouts/chapter_1_4_handout.xlsx" download="chapter_1_4_handout.xlsx"> Tables-Handout-Excel </a>
:::
...
<a href="handouts/chapter_1_4_handout_key.xlsx" download="chapter_1_4_handout_key.xlsx"> chapter_1_4_handout_key.xlsx </a>
