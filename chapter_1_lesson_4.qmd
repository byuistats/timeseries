...
deaths_shat_ts |>
  ...
  mutate(
    s_hat = ifelse(row_number() <= 14, "", s_hat),
...
# FIX THIS NOTATION .... FIX THIS NOTATION .... FIX THIS NOTATION .... FIX THIS NOTATION .... 

Next, we compute the mean of $\hat s_t$ for all values within each month. Call these values $\bar {\hat s_t}$. 

The mean of the $\bar {\hat s_t}$ values will be reasonably close to, but not exactly zero. We adjust these values by subtracting the overall mean from $\bar {\hat s_t}$ for each of the months to get the seasonally adjusted mean:

$$
  \bar s_t = \hat s_t - \bar {\hat s_t}
$$

where $\bar {\hat s_t}$ is the average of the $\hat s_t$ values corresponding to month $t$.
...
Next, we need to compute the seasonally adjusted mean. This is the mean (across years) of $\hat s_t$ for each month. To compute this, it can be convenient to organize the values of $\hat s_t$ in a table, where the rows give the year and the columns give the month.
...
# CALCULATE THE RANDOM TERM AND PLOT THE DECOMPOSITION....................ADD THIS.....................
...
rplot <- autoplot(adjusted_ts, .vars = random) +
  # geom_line(aes(x = index, y = seasonally_adjusted), color = "#D55E00") +
  # geom_line(aes(x = index, y = m_hat), color = "#0072B2") +
  labs(
    x = "Month",
    y = "Traffic Fatalities",
    title = "random: Traffic Fatalities in the United States (by Month)"
  ) +
  # coord_cartesian(ylim = c(0,4500)) +
  theme(plot.title = element_text(hjust = 0.5))
...
adjusted_ts |> 
  ...
  mutate(
    ...
    adjusted_s_bar = cell_spec(adjusted_s_bar, color = "#009E73"),
    random = cell_spec(random, color = "#009E73"),
    seasonally_adjusted = cell_spec(seasonally_adjusted, color = "#009E73")
  ) |>
  display_table()
```
