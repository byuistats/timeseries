---
title: "Holt-Winters Method (Additive Model) - Part 2"
...
## Introduction to the Holt-Winters Method--Additive Model (30 min)
...
## Small Group Activity: Holt-Winters Model for Residential Natural Gas Consumption (30 min)
...
We will use this information to create an initial estimate of the seasonality of the time series. We will assume that in Quarters 1 and 4, natural gas use is 1000 Bcf above the level of the time series and in Quarters 2 and 3, it is 1000 Bcf below the level of the time series. 
...
nat_gas_ts |>
  ...
  mutate(
    x_t = ifelse(t %in% c(-3:0, 28:nrow(nat_gas_ts)), "—", x_t),
    a_t = ifelse(t %in% c(-3:0, 28:nrow(nat_gas_ts)), "—", a_t),
    b_t = ifelse(t %in% c(-3:0, 28:nrow(nat_gas_ts)), "—", b_t),
    xhat_t = ifelse(t %in% c(-3:0), "—", xhat_t)
  ) |>
  rename(
    "$$Quarter$$" = date,
    "$$t$$" = t,
