...
### IDEA FOR REVISION:
#### Have students compute the monthly means
#### Get decomposition in R for monthly mean:
```{r}
# Weather data for Rexburg
...
monthly_decompose <- monthly_tsibble  |>
  model(feasts::classical_decomposition(average_daily_high_temp,
                                        type = "add"))  |>
  components()


monthly_decompose |> autoplot(.vars = seasonal)
```

#### Then, provide assistance to get daily decomposition (including the ~ season(365.25) command)
```{r}
# Weather data for Rexburg
...
#### Have students compare the two decompositions. Which is better? Why?
