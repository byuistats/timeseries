...
ggplotly(ggplot(data = df, aes(x = seq_along(x), y = acf(x, plot = FALSE)$acf)) +
  # geom_col() +
  ylim(-1, 1) +
...
autoplot(choc_decompose) |> ggplotly()
```
...
```{r}
result <- acf(choc_decompose$random |> na.omit(), plot=FALSE, type = "correlation", lag.max = 25)
...
ggplotly(ggplot(acf_plot_data, aes(x = lag, y = acf)) +
...
```{r}
ggplotly(ACF(enrollment_decompose$enrollment, type = "correlation"))
```
