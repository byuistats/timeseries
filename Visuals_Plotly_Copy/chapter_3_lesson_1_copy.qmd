...
df <- data.frame(x = -4:4)
ggplotly(ggplot(data = df, aes(x = x, y = acf(x, plot = FALSE)$acf)) +
  # geom_col() +
  ylim(-1, 1) +
...
ggplotly(autoplot(housing, .vars = starts) +
...
ggplotly(autoplot(random_joint, .vars = random_starts) +
...
ggplotly(ggplot(data = rexburg_daily_ts, aes(x = max_solar_angle, y = high_temp)) +
  geom_point() +  
  labs(
...
ggplotly(ggplot(data = rexburg_daily_ts, aes(x = lag(max_solar_angle, 28), y = high_temp)) +
  geom_point() +  
  labs(
