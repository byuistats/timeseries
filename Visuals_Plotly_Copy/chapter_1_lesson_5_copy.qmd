...
trend_plot <- ggplotly(ggplot(x_df_ts, aes(x=month, y=trend)) + 
                         geom_line() +
                         labs(title="", 
                              x=NULL, 
                              y="TREND") +
...
seasonal_plot <- ggplotly(ggplot(x_df_ts, aes(x=month, y=seasonal)) + 
                             geom_line() +
                             labs(title="", 
                                  x=NULL, 
                                  y="Seasonal") +
...
error_plot <- ggplotly(ggplot(x_df_ts, aes(x = month, y = x_t - trend - seasonal)) + 
                          geom_line() +
                          labs(title="", 
                               x=NULL, 
                               y="Random") +
...
x_plot <- ggplotly(ggplot(x_df_ts, aes(x=month, y=x_t)) + 
                      geom_line() +
                      labs(title="True (Simulated) Values", 
                           x=NULL,  
                           y="$$x_t$$") +
                      theme(plot.title = element_text(hjust = 0.5)))
...
fig <- subplot(x_plot, trend_plot, seasonal_plot, error_plot, nrows = 4, titleY = TRUE, titleX = TRUE )
...
ggplotly(ggplot(df, aes(x=i, y=x_t)) + 
  geom_line() +
...
rexburg_weather_plot <- ggplotly(autoplot(rexburg_daily_ts, .vars = value) +
  labs(
    x = "Date",
    y = "High Temp (F)",
...
sp500_plot <- ggplotly(autoplot(sp500_ts, .vars = value) +
  labs(
    x = "Date",
    y = "Closing Price",
...
fig <- subplot(rexburg_weather_plot, sp500_plot, nrows = 2, titleY = TRUE, titleX = TRUE )
...
ggplotly(autoplot(apple_ts, .vars = revenue) +
  labs(
    x = "Quarter",
...
ggplotly(autoplot(apple_decompose))
```
