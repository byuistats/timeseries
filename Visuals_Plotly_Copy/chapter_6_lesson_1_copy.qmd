...
gdp_df <- as.data.frame(gdp_ts)

ggplotly(ggplot(gdp_df, aes(x = index(gdp_ts), y = gdp_millions)) +
  geom_line() +
  labs(
    x = "Quarter",
...
ggplotly(ggplot(gdp_df, aes(x = index(gdp_ts), y = year_over_year)) +
  geom_line() +
...
vessels_ts <- as.data.frame(vessels_ts)

ggplotly(ggplot(vessels_ts, aes(x = index(vessels_ts), y = ratio)) +
  geom_line() +
  labs(
    x = "Month",
    y = "Ratio",
...
vessels_ts <- as_tsibble(vessels_ts, index = date)
