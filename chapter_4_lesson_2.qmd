...
\begin{align*}
  \nabla^2 x_8 &= \nabla \left( \nabla x_8 \right) \\
    &= (x_8-x_7)-(x_7-x_6)
\end{align*} 
and that these are equal to the last term in the sequence of second differences.
...
and that this is equal to the last term in the sequence of second differences.
...
Z.hw <- HoltWinters(stock_ts0, alpha = 1, gamma = FALSE)
Z.hw
```
...
\begin{cases}
~~~x_t     = x_{t-1} + b_{t-1} + w_t \\
   b_{t-1} = `r Z.hw$beta |> round(3)` \left( x_{t-1} - x_{t-2} \right) + `r 1 - Z.hw$beta |> round(3)` ~ b_{t-2} 
\end{cases}
$$
...
$$
  (1 - \mathbf{B})^2 x_t = (1 - `r 1 - Z.hw$beta |> round(3)`) w_t
$$
...
~~~x_t     = \mathbf{B} x_{t} + \mathbf{B} b_{t} + w_t \\
   \mathbf{B} b_{t} = `r Z.hw$beta |> round(3)` \left( \mathbf{B} x_{t} - \mathbf{B}^2 x_{t} \right) + `r 1 - Z.hw$beta |> round(3)` ~ \mathbf{B}^2 b_{t} 
\end{cases}
$$
...
\begin{align*}
  \mathbf{B} b_{t} 
    &= `r Z.hw$beta |> round(3)` \left( \mathbf{B} x_{t} - \mathbf{B}^2 x_{t} \right) + `r 1 - Z.hw$beta |> round(3)` ~ \mathbf{B}^2 b_{t}  \\
    &= `r Z.hw$beta |> round(3)` \left(  1 - \mathbf{B} \right) \mathbf{B} x_{t}   + `r 1 - Z.hw$beta |> round(3)` ~ \mathbf{B} \left[ \mathbf{B} b_{t} \right]  
\end{align*}
Substitutingthe expression for $\mathbf{B} b_{t}$ into the second equation:
\begin{align*}
  (1-\mathbf{B}) x_t - w_t 
    &= `r Z.hw$beta |> round(3)` \left(  1 - \mathbf{B} \right) \mathbf{B} x_{t}   + `r 1 - Z.hw$beta |> round(3)` ~ \mathbf{B} \left[ (1-\mathbf{B}) x_t - w_t \right]  \\
    &= `r Z.hw$beta |> round(3)` \left(  1 - \mathbf{B} \right) \mathbf{B} x_{t}   + `r 1 - Z.hw$beta |> round(3)` ~ \mathbf{B} (1-\mathbf{B}) x_t - `r 1 - Z.hw$beta |> round(3)` ~ \mathbf{B} w_t \\
    &= \left(  1 - \mathbf{B} \right) \mathbf{B} x_{t} - `r 1 - Z.hw$beta |> round(3)` ~ \mathbf{B} w_t   \\
\end{align*}
...
\begin{align*}
  ...
  (1-\mathbf{B}) x_t - \left(  1 - \mathbf{B} \right) \mathbf{B} x_{t}
    &=  w_t - `r 1 - Z.hw$beta |> round(3)` ~ \mathbf{B} w_t   \\
  \left( 1-\mathbf{B} \right) \left(  1 - \mathbf{B} \right) x_{t}
    &=  \left( 1 - `r 1 - Z.hw$beta |> round(3)` ~ \mathbf{B} \right) w_t   \\
