...
df_quadratic <- data.frame(t = 1:10) |>
  mutate(x = 2 * (t - 5.5)^2 - 4) |>
  mutate(diff1 = x - lag(x)) |>
...
## Class Activity: Random Walk with Drift (10 min)
...
We will now consider the daily closing price of Abercrombie & Fitch stock. Here is a time series plot of these stock from 
...
This confidence interval contains 0, so we cannt conclude that there is evidence of a positive drift in the price of McDonald's stock over this period.
