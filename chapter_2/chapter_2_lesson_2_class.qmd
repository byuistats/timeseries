...
make_cov_table_df <- function(df, offset=1, decimals_1st_order = 4, decimals_2nd_order = 4) {
  ...
  df %>%
    ...
    mutate(
      x = ifelse(row_number() <= offset, paste0("[",x,"]"), x),
      y = ifelse(row_number() <= offset, NA, y),
    ) %>% 
    replace(., is.na(.), "—") %>% 
...
## Hands-on Exercise -- Exploring Autocorrelation (40 min)
<!-- Compute sample acvf and acf -->

In the previous lesson, we computed the sample covariance and sample correlation coefficient between two independent variables. When working with time series, there is often a correlation between sequential observations. We 
...
cov_table <- cov_df %>%
  make_cov_table_df(offset = offset_value) 
cov_table %>%
  display_cov_table()
```
In this case, $y_i$ corresponds to $x_{i-1}$, where $i>1$. The covariance of $x_i$ and $x_{i-1}$ is: 

$$
  cov(x,y) = \frac{\sum\limits_{i=`r offset_value + 1`}^{11}(x_i-\bar x)(y_i-\bar y)}{n-1} 
= \frac{`r as.numeric(cov_table[nrow(cov_table),8])`}{`r nrow(cov_df %>% na.omit())` - 1}
= `r compute_cov(cov_df)`
$$ 

This is the covariance of $x$ with itself, but with a lag of 1 time unit. This is called the **lag 1 autocovariance** or the **autocovariance of $x$ with lag 1**.

-   What does the lag 1 autocorrelation measure?

We can compute the **lag 1 autocorrelation** or the **autocorrelation of $x$ with lag 1** by dividing the autocovariance by the square roots of the variances of $x_i$ and $x_{i-1}$:
...
\begin{align*}
  ...
  =
  \frac{ \frac{\sum\limits_{i=`r offset_value + 1`}^{11}(x_i-\bar x)(y_i-\bar y)}{n-1}
      }{
        \sqrt{
          \frac{\sum\limits_{i=`r offset_value + 1`}^{11}(x_i-\bar x)^2}{n-1}
        }
        \sqrt{
          \frac{\sum\limits_{i=`r offset_value + 1`}^{11}(y_i-\bar y)^2}{n-1}
        }
      }  
  \\
  &= 
  \frac{ \sum\limits_{i=`r offset_value + 1`}^{11}(x_i-\bar x)(y_i-\bar y)
      }{
        \sqrt{
          \sum\limits_{i=`r offset_value + 1`}^{11}(x_i-\bar x)^2
        }
        \sqrt{
          \sum\limits_{i=`r offset_value + 1`}^{11}(y_i-\bar y)^2
        }
      } 
  =
  \frac{
...
```{r}
# cov_df2 <- get_data_for_cov_table(offset = 2)
# cov_df2 %>%
#   make_cov_table_df(offset = 2, decimals_1st_order = 1, decimals_2nd_order = 2) %>% 
#   make_blank_row_in_cov_table(9) %>%
#   make_blank_row_in_cov_table(10) %>%
#   make_blank_row_in_cov_table(11) %>%
#   make_blank_row_in_cov_table(12) %>%
#   display_cov_table()
```
```{r}
offset_value <- 2
