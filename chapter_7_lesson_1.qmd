...
## Class Activity: Non-seasonal ARIMA Models (xxx min)
...
$\theta_p \left(\mathbf{B} \right) y_t = \phi_q \left(\mathbf{B} \right)$.
...
## Class Activity: Fitting an ARIMA Process (xxx min)

The data file [exchange_rates.parquet](https://byuistats.github.io/timeseries/data/exchange_rates.parquet) gives the exchange rates for foreign currencies. Each value is the amount in the foreign currency that is equivalent to one U. S. dollar.

```{r}
#| code-fold: true
#| code-summary: "Show the code"

# BEAUTIFUL ARIMA Model
# EXCHANGE RATES: USD.EUR (1 USD = ? EUR)
...
acf(exchange_ts$rate, main = paste("ACF of", exchange_ts$currency[1]))

pacf(exchange_ts$rate, main = paste("ACF of", exchange_ts$currency[1]))
...
acf(model_resid$.resid, main = paste("ACF of", exchange_ts$currency[1]))

pacf(model_resid$.resid, main = paste("ACF of", exchange_ts$currency[1]))

hist(model_resid$.resid)
```
...
combined_models <- stock_model |>
  glance() |>
  select(-ar_roots, -ma_roots)

extrema <- combined_models |>
  reframe(
