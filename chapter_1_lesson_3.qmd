...
| 7\. Centered moving average for observations made monthly                                               | G. $\hat s_t = \dfrac{x_t}{\hat m_t}$ |
...
## Team Activity: Moving Averages (15 min)
...
-   Give a formula that combines the two means above to give one mean that is centered on an integer value of $t$. Do not include a summation symbol in your formula. (Hint: try averaging the two means.)
-   Upon what value of $t$ is your new mean centered?

:::

We will now adjust this moving average, so it is centered on any given value of $t$, not just $t=7$.
...
### Application: Google Trends Searches for "Chocolate" (30 min)
...
moving_avg <- autoplot(chocolate_month_ts, .vars = m_hat) +
  labs(
    x = "Month",
    y = "Searches",
...
superimposed_moving_avg <- autoplot(chocolate_month_ts, .vars = value) +
  labs(
    x = "Month",
    y = "Searches",
...
moving_avg | superimposed_moving_avg
```
...
| 7\. Centered moving average for observations made monthly                                               | D. $\hat m_t$                         |
