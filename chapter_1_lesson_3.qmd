...
-   Review Sections 1.4.1 and 1.5.3
    -   Pay particular attention to the seasonal box plots and the centered moving average
...
This code can be used to compute the moving average over 12 months:
...
chocolate_month_ts |>
  mutate( m_hat = 
        ( 
            1/2 * lag(value,6) + lag(value,5) + lag(value,4) + lag(value,3) 
            + lag(value,2) + lag(value) + value + lead(value,1) + lead(value,2) 
            + lead(value,3) + lead(value,4) + lead(value,5) + 1/2 * lead(value,6)
        ) / 12
  )
```
