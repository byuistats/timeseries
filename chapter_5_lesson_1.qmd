...
-   Which of the following models are linear?
    ...
    -   Model 1:
$$
  x_t = \alpha_0 + \alpha_1 \sin(x_{t-1}) + \alpha_2 e^{t} + z_t
$$
...
$$
    ...
    -   Model 8:
$$
  x_t = \alpha_0 \sin( \alpha_1 + \alpha_2 t + z_t )
$$
...
# Make this number large (like 1000) when the document is ready to be compiled for the last time.
# Make this number large (like 1000) when the document is ready to be compiled for the last time.
# Make this number large (like 1000) when the document is ready to be compiled for the last time.
# Make this number large (like 1000) when the document is ready to be compiled for the last time.
# Make this number large (like 1000) when the document is ready to be compiled for the last time.
...
```{r}
#| label: Simulation...this takes a minute
#| code-fold: true
#| code-summary: "Show the code"
...
|:---------:|:--------:|:---------:|:---------:|
| $\alpha_0$ | `r parameter_est[1,1] |> round(3)` | `r stderr[1] |> round(3)` | `r standard_errors$alpha0 |> round(3)` |
| $\alpha_1$ | `r parameter_est[1,2] |> round(3)` | `r stderr[2] |> round(3)` | `r standard_errors$alpha1 |> round(3)` |

: Autocorrelation function and partial autorcorrelation function of the residuals from the fitted linear model for the simulated data. {#tbl-simParameters}
...
retail_df3 |>
  ...
  replace_cells_with_char(rows = 1:nrow(retail_df3), cols = 4:7) |>
  display_partial_table(14,8,"1in") |>
