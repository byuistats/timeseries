...
last_full_year <- weather_monthly |> 
  ...
  select(year) |>
  pull()
last_full_year <- 2023 # Comment out this line, if there is a positive slope in future years. Otherwise, this assures that the positive slope from 2022 to 2023 is illustrated. Note that this overwrites the results of the last command

last_2_years <- weather_monthly |>
  filter(year == last_full_year | year == last_full_year - 1) |>
  mutate(month_seq = 1:24)

# Fit regression model
model <- lm(avg_daily_high_temp ~ month_seq, data = last_2_years)

# Create data frame with line that goes through origin 
# and is parallel to regression line
line_df <- data.frame(month_seq = c(0,24.5)) |>
  mutate(avg_daily_high_temp = coef(model)[2] * month_seq)

arrow_df0 <- last_2_years |>
  mutate(
    arrow_start = coef(model)[2] * month_seq,
    deviation = avg_daily_high_temp - arrow_start
  ) 
...
ggplot(last_2_years, aes(month_seq, avg_daily_high_temp)) +
  geom_point(color = "#56B4E9", size = 2) + 
  geom_line(data = line_df, color = "#D55E00") +
  ...
  geom_segment(aes(x = month_seq, xend = month_seq, 
               ...
               arrow = arrow(length = unit(0.25, "cm")),
                             color = "#009E73") +
  geom_segment(x = 0, xend = 24.7, y = 0, yend = 0, 
               arrow = arrow(length = unit(0.25, "cm"))) +
  geom_segment(x = 0, xend = 0, y = 0, yend = 90,
               arrow = arrow(length = unit(0.25, "cm"))) +
  geom_text(x = 24.75, y = 3, label = "x") +
  geom_text(x = 0.375, y = 92, label = "y") +
  scale_x_continuous(breaks = 1:24) +
  labs(
    x = "Month", 
    y = "Temperature (Fahrenheit)",
    title = paste0(
      "Mean Monthly High Temperature in Rexburg (", 
      min(last_2_years$year), 
      "-", 
      max(last_2_years$year), 
      ")")
  ) +
  theme_minimal() +
