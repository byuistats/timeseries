...
The autocorrelation in the data show that OLS is insufficient on its own. To fix it, we can add an AR(p) process to the residuals.

$$ y_t = \beta_0 + \beta_1 x_t + \eta_t$$
where where $\eta_t$ is an $AR(2)$ error. Note that $\eta_t$ can represent more than just $AR$ processes, but we have specified it here as an $AR(2)$.

We can easily add an AR process to the residuals with the following code.
