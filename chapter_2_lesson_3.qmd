...
Use these values to sketch the correlogram for these data in your Learning Journal. The figure below can help you begin.
...
```{r}
acf(chocolate_month_ts$value, plot=FALSE, type = "correlation")
```
...
-   Is there a trend in the chocolate search data? If so, speculate about the cause of this trend.
...
## Small Group Activity: BYU-Idaho On-Campus Enrollment (30 min)

The official number of on-campus BYU-Idaho students each semester is given in the file <a href="data/byui_enrollment.csv" download>byui_enrollment.csv</a>. 
...
# read in the data from a csv and make the tsibble
# change the line below to include your file path
enrollment_df <- rio::import("data/byui_enrollment.csv")
start_date <- lubridate::ymd("2019-05-01")
date_seq <- seq(start_date,
                start_date + months(nrow(enrollment_df)-1) * 4,
                by = "4 months")
enrollment_tibble <- tibble(
  dates = date_seq,
  year = lubridate::year(date_seq),
  month = lubridate::month(date_seq),
  value = pull(enrollment_df, enrollment)
)
enrollment_ts <- enrollment_tibble |>
  mutate(index = tsibble::yearmonth(dates)) |>
  as_tsibble(index = index)
...
acf(enrollment_decompose$value, type = "correlation")
acf(enrollment_decompose$value, plot=FALSE, type = "correlation")
acf(enrollment_decompose$value, plot=TRUE, type = "correlation")
...
```{r}
#| eval: false
rio::import("https://byuistats.github.io/timeseries/data/byui_enrollment.csv")
```


::: {.callout-tip icon=false title="Check Your Understanding"}

Working with your partner, compute the values of the acf for these data. 

-   What does the autocorrelation plot 
:::
...
<a href="https://tbrost.github.io/BYUI-Timeseries-Drafts/Homework/HW-Draft.qmd" download="TS_HW_3_2.qmd"> Chapter 2 Lesson 3 Homework </a>
