...
```{r}
ggplot(stock_ts, aes(x = dates, y = value)) +
  geom_line(color = "steelblue", linewidth = 1) +
  scale_y_continuous(
    labels = scales::label_dollar()
  ) +
  labs(
    title = "Tesla (TSLA) Daily Adjusted Closing Prices",
    subtitle = "From July 2020 to January 2024",
    x = "",
...
```{r}
autoplot(stock_ts_diff, .vars = first_diff) +
  labs(
    title = "Time Plot of First-Differenced Tesla Daily Closing Prices",
    x = "",
    y = "First Difference of Closing Price (US$)"
  ) +
  theme_minimal()
```
