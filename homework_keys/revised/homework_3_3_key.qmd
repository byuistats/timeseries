...
df <- rio::import("https://byuistats.github.io/timeseries/data/ILLNESS_CDCWONDER.csv") |>
  filter(`ICD Sub-Chapter Code` == "X60-X84")|> 
  mutate(yearmonth = yearmonth(Date))

df2 <- read_csv("https://byuistats.github.io/timeseries/data/POPTHM.csv") |> 
  mutate(yearmonth = yearmonth(observation_date))

illness <- df |> 
  left_join(df2, by="yearmonth") |>
...
tidy(illness_hw) |> select(!.model) |> head(3) |> pander()

# Display fit statistics
glance(illness_hw) |> select(!.model) |> pander()
```

Tip: Use `?fable` or look at the [documentation](https://fable.tidyverts.org/reference/index.html) to find new functions you can use.
...
```{r}
# Display model parameters
params <- illness_hw$Additive[[1]]$fit$par
pander(tibble(
  Term = params$term, 
...
# Display fit statistics
pander(illness_hw$Additive[[1]]$fit$fit)
```

If you want to have fun exploring, run `illness_hw$Additive[[1]]`

Then use class() and str() as you descend deeper

:::

```{r}
illness_fitted <- augment(illness_hw)

ggplot(illness_fitted, aes(x = yearmonth, y = `Deaths per 100000`)) +
...
#   (like a tsibble for forecasts, with extra stuff like confidence intervals)
illness_forecast <- forecast(illness_hw, h = "24 months")

# Create a tsibble with fitted values (.fitted) from holt winters model
illness_fitted <- augment(illness_hw)

# Plot   blue line,     black line, blue band
autoplot(illness_forecast, illness_ts, level = 95) + 
  geom_line(
    data = illness_fitted,
  ...
  theme_bw() +
  labs(
    title = "24 Month Forecast of Deaths by Suicide in the U.S.",
    subtitle = "Fitted with a Holt-Winters decomposition",
    x = NULL, 
    y = "Deaths per 100,000",
    caption = "Data attributed to CDC Wonder
...
# Zoomed out view
far <- autoplot(illness_forecast, level = 95) + # blue line
  geom_line(
    data = illness_current, 
    aes(x = yearmonth, y = `Deaths per 100000`, color = "Actual") # black line
  ) +
  geom_line(
    data = illness_fitted,  # convert to tsibble with .fitted column
    aes(y = .fitted, color = "Fitted"), # orange line
  ) +
  ...
  labs(
    ...
    y = "Deaths per 100,000", 
    color = "", 
    title = "Suicide Deaths 1999-2024"
  ) +
  theme_bw() +
  theme(legend.position = "bottom")

# Zoomed in view
close <- autoplot(illness_forecast, level = 95) +
  geom_line(
    data = illness_current, 
  ...
  scale_color_manual(values = c("Fitted" = "orange", "Actual" = "black")) +
  scale_x_yearmonth(limits = c(ym("2018-01"), ym("2023-12"))) +
  scale_y_continuous(limits = c(1, 1.4)) +
  labs(
    x = NULL, 
    y = "Deaths per 100,000", 
    color = "", 
    title = "Suicide Deaths 2018-2024",
    caption = "Data attributed to CDC Wonder
               https://wonder.cdc.gov/wonder/help/ucd.html"
  ) +
  theme_bw() +
...
# Put it together and what do you got? bippity-boppity-view
far / close
```
