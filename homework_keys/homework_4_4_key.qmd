...
**Meaning **
Each observation represents the average exchange rate for the dollar-to-euro conversion for a specific business day, providing insights into the currency’s relative strength and market conditions on that date.
:::
...
# Generate all random noise
w <- rnorm(n, sd = sqrt(sigma_sqr))

# Place the first two values
x <- rep(0, n)
x[1] <- alpha0 + w[1]
x[2] <- alpha0 + alpha1 * (x[1] - alpha0) + w[2]

# Populate the rest of the series
for (i in 3:n) {
  x[i] <- alpha0 + alpha1 * (x[i-1] - alpha0) + alpha2 * (x[i-2] - alpha0) + w[i]
}

# Bind to a tibble and cast to tsibble at the very end
sim_ts <- tibble(index = 1:n, x = x) |> 
...
::: {.callout-note title="Answer" icon="false"}
```{r}
# Turn simulation into a function for easier use
simulate_ar2 <- function(seed, n) {
  set.seed(seed)
...
```{r}
simulate_ar2(123, 100)
```

#### Seed 164

```{r}
simulate_ar2(164, 100)
```

#### Seed 198

```{r}
simulate_ar2(198, 100)
```
...
```{r}
simulate_ar2(123, 1000)
```

#### Seed 164

```{r}
simulate_ar2(164, 1000)
```

#### Seed 198

```{r}
simulate_ar2(198, 1000)
```
...
gg_tsdisplay(us_ts, plot_type = "partial") +
  labs(
...
autoplot(us_ts) +
  labs(
    title = "USD/EUR Spot Exchange Rate",
    x = "Date",
    y = "USD per Euro"
  ) +
  theme_bw()
...
```{r, warning = FALSE}
diff_plot <- autoplot(us_ts, diff) +
  labs(
    title = "First-Differenced USD/EUR Exchange Rate",
    x = NULL,
    y = "Δ Exchange Rate"
  ) +
  theme_minimal()
...
diff_pacf <- us_ts |>
  ...
  autoplot() +
  labs(
    title = "Partial Correlogram (PACF)"
  ) +
  theme_minimal()

diff_plot / (diff_acf + diff_pacf)
```
...
ggplot(data = us_ts, aes(x = date, y = diff)) +
  ...
  labs(
    ...
    title = "First difference of US-Euro Exchange Rate"
  ) +
  theme_bw()
