...
kw <- rio::import("https://byuistats.github.io/timeseries/data/USENERGYPRICE.csv")
...
value <- (kw_ts$usenergyprice[82]+kw_ts$usenergyprice[84])/2
# this evaluates to 0.835

# Fill missing value with average of surrounding values
kw_ts$usenergyprice[83] <- value

```

For your convenience the data provided for this Homework assignment has already been filled.
...
autoplot(components(kw_hw))

kw_fitted <- augment(kw_hw) |> 
  filter(date > yearmonth("2000 Nov"), date < yearmonth("2016 Dec"))

# augment to get .fitted
ggplot(kw_fitted, aes(x = date, y = usenergyprice)) +
...
```{r}
hw <- model(kw, Additive = ETS(usenergyprice ~ trend("A", alpha=0.8)))

autoplot(components(hw))
```
...
To select smoothing parameters, I allowed the model to choose the initial estimates. After plotting a correlogram of the error components, I noticed a significant trend remaining. I increased the alpha to put heavier weight on the more recent level. Adjusting the other values didn’t yield any more improvements, so I left them as is.

```{r}
acf(components(hw)$remainder |> na.omit())
report(hw)
```
...
```{r}
autoplot(forecast(hw, h = "12 months"), kw) +
  geom_line(data = augment(hw), aes(y = .fitted), color = "orange") +
  theme_bw()

autoplot(forecast(hw, h = "12 months"), kw) +
  geom_line(data = augment(hw), aes(y = .fitted), color = "orange") +
  scale_x_yearmonth(limits = c(ym("2020-01"), ym("2026-01"))) +
  theme_bw()
```
...
real_kw <- rio::import("https://byuistats.github.io/timeseries/data/USENERGYPRICE_Real.csv")
...
::: {.callout-note title="Answer" icon="false"}
```{r}
# Did you catch that this data is in quarters, not months?
real_kw_ts <- real_kw |> 
...
```{r}
autoplot(components(real_kw_hw))
augment(real_kw_hw) |>
...
```{r}
real_hw <- real_kw |> 
  model(ETS(realprices ~ trend("A", alpha = 0.2, beta = 0.01) + 
                         error("A") + 
                         season("A", gamma = 0.2)
            )
        )

autoplot(components(real_hw))
```
...
While I initially considered using the correlogram to remove all patterns from the remainder component, I thought of the value that looking at the remainder component could bring in identifying events of interest. I attempted to pick an alpha for a reasonably smooth level. The seasonality seems to change over time, especially in more recent data, so I made sure to keep a high enough gamma value. For beta, I just chose a value that made it reasonably smooth, but not exactly the same as the level.

```{r}
acf(components(real_hw)$remainder |> na.omit())
report(real_hw)
```

## Dallin

I kept the smoothing parameters the same. Much remains true from the previous model, and since the main change from the previous data to the real price data was in the slope, which was already extremely low.
...
autoplot(forecast(real_hw, h = "12 months"), real_kw) +
  geom_line(data = augment(real_hw), aes(y = .fitted), color = "orange") +
  scale_x_yearmonth(limits = c(ym("2020-01"), ym("2026-01"))) +
  theme_bw()
...
While the data is more stochastic when removing the affects of dollar value, there are still identifiable patterns.

#### Trend
The trend varies stochastically, but it seems to stay fairly level around $0.15. The slope echos that idea that while it varies randomly, it remains fairly level around 0 slope.

#### Seasonality
Seasonality is still very present in the data, but it does seem to change in degree more over time. It seems to diminish over time, which could be due to changes in technology or prediction that allow price to remain more constant throughout the year.
:::
