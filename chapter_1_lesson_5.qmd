...
autoplot(apple_ts, .vars = revenue) +
  labs(
    ...
    title = "Quarterly Apple Revenue, Billions of U.S. Dollars"
  ) +
  theme(plot.title = element_text(hjust = 0.5))
```
...
We to find an estimate for the time series that does not fluctuate with the seasons. This is called the seasonally adjusted series.

### Centered Moving Average

First, we compute the centered moving average, $\hat m_t$. This code can be used to compute the four-quarter centered moving average.
...
apple_shat_ts |>
  ...
  color_specific_cell(row_num = 7, col_num = 5, color = "#009E73") |>
  color_specific_cell(row_num = 5, col_num = 4, color = "#009E73") |>
  color_specific_cell(row_num = 6, col_num = 4, color = "#009E73") |>
  color_specific_cell(row_num = 7, col_num = 4, color = "#009E73") |>
  color_specific_cell(row_num = 8, col_num = 4, color = "#009E73") |>
  color_specific_cell(row_num = 9, col_num = 4, color = "#009E73") |>
  display_partial_table(14, 8)
```
...
$$
  \bar s_t = \bar {\hat s_t} - \left( \bar{\bar {\hat s_t}} + 1 \right)
$$
...
#### Table of $\hat s_t$ values for each quarter
...
-   The table above gives the values of $\hat s_t$. Fill in the missing values in the first two rows of table above. (Note you already computed these.)
...
-   Divide each of the $\bar {\hat s_t}$ values by $\bar {\bar {\hat s_t}}$ to get $\bar s_t$, the seasonally adjusted mean for month $t$. (Note that the mean of the $\bar s_t$ values is 1.)
...
Using R's definition of the multiplicative model, we calculate the random component by dividing the values in the time series by the product of the trend and the seasonally adjusted mean:
...
three_more_rows_from_adjusted_ts <- head(adjusted_ts,7+3) |> 
  tail(3) |> 
...
autoplot(crime_monthly_ts, .vars = value) +
  labs(
    ...
    title = "Total Monthly Crime Incidents"
  ) +
  theme(plot.title = element_text(hjust = 0.5))
```
