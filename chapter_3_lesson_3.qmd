...
The United States Energy Information Administration (EIA) [publishes data](https://www.eia.gov/dnav/ng/ng_cons_sum_dcu_nus_m.htm) on the total residential natural gas consumption in the country. 
...
nat_gas <- rio::import("data/natural_gas_consumption_residential_us.csv") |>
  ...
  group_by(quarter) |> 
  summarize(
    gas_use = sum(residential_nat_gas_consumption),
    n = n()
  ) |>
  filter(n == 3) |>
  select(-n)
...
nat_gas_ts <- nat_gas |>
  filter(quarter > my("Dec 2016")) |>
  mutate(billion_cf_gas = round(gas_use / 10^3 ) ) |>
  hw_additive_slope_additive_seasonal_gas("quarter", "billion_cf_gas", p = 4, predict_periods = 12, s_initial = c(1000, -1000, -1000, 1000)) |>
  as_tsibble(index = date) |> 
  dplyr::select(date, x_t, a_t, b_t, s_t, xhat_t)
