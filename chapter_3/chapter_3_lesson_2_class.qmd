...
-   What do the following terms represent?
    ...
    -   $p$
    -   $\dfrac{q N_t}{m}$
-   Starting with Equation (3.6), solve for $\dfrac{N_{t+1} - N_t}{m-N_t}. 
...
\begin{align*}
  N_{t+1} 
    &= m \dfrac{ 1 - e^{-(p+q)(t+1)} }{ 1 + (q/p) e^{-(p+q)(t+1)} } \\
    &= m \dfrac{ 1 - e^{-(p+q)t}e^{-(p+q)} }{ 1 + (q/p) e^{-(p+q)t}e^{-(p+q)} } 
\end{align*}
\begin{align*}
  N_{t+1} 
    &= N_t + p(m-N_t) + q N_t (m-N_t) / m \\
    &= \left[ m \dfrac{ 1 - e^{-(p+q)t} }{ 1 + (q/p) e^{-(p+q)t} } \right] + p\left(m-\left[ m \dfrac{ 1 - e^{-(p+q)t} }{ 1 + (q/p) e^{-(p+q)t} } \right]\right) + q N_t \left(m-\left[ m \dfrac{ 1 - e^{-(p+q)t} }{ 1 + (q/p) e^{-(p+q)t} } \right]\right) / m 
\end{align*}
Let's simplify a common term:
\begin{align*}
  \left(m-\left[ m \dfrac{ 1 - e^{-(p+q)t} }{ 1 + (q/p) e^{-(p+q)t} } \right]\right)
    &= m\left(1-\left[ \dfrac{ 1 - e^{-(p+q)t} }{ 1 + (q/p) e^{-(p+q)t} } \right]\right) \\
    &= m\left(\dfrac{ \left[ 1 + (q/p) e^{-(p+q)t} \right]-\left[  1 - e^{-(p+q)t} \right] }{ 1 + (q/p) e^{-(p+q)t} } \right) \\
    &= m\left(\dfrac{ (q/p) e^{-(p+q)t} + e^{-(p+q)t} }{ 1 + (q/p) e^{-(p+q)t} } \right) \\
\end{align*}
:::