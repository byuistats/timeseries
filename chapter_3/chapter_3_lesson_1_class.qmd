...
acf_temp <- ACF(rexburg_daily_ts, y = value) |> autoplot() +
    labs(title = "Daily High Temperature")
joint_ccf_plot <- rexburg_daily_ts |>
  CCF(y = max_solar_angle, x = value) |> autoplot() +
  labs(title = "CCF Plot")
...
```{r}
#| eval: false
solar_decompose <- model(rexburg_daily_ts, feasts::classical_decomposition(max_solar_angle)) |>
    components()
temp_decompose <- model(rexburg_daily_ts, feasts::classical_decomposition(value)) |>
    components()
