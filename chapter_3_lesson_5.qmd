...
```{r}
# MOVE TO COMMON FUNCTIONS WHEN SAFE

hw_additive_slope_multiplicative_seasonal <- function(df, date_var, value_var, p = 12, predict_periods = 18, alpha = 0.2, beta = 0.2, gamma = 0.2, s_initial = rep(0,p)) {
...
## Class Discussion: Variations on the Holt-Winters Model (10 min)
...
When is each of these models appropriate? 

-   Trend:
    -   When the trend is linear, an additive trend term is appropriate. 
    -   If the trend shows exponential growth, a multiplicative trend should be applied. 
-   Seasonality:
    -   If the variation from one season to the next is roughly constant, then an additive seasonality component is appropirate.
    -   If the variation increases as the level increases, then a multiplicative seasonality term is appropriate.
...
-   Which of the four Holt-Winter models described above is appropriate for this situation? Justify your answer.
