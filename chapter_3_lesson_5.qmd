...
: Holt-Winters forecasting equations for different models {#tbl-forecast-models}
...
byui_enrollment_ts |>
  ggplot(aes(x = dates, y = enrollment_1000)) +
...
@tbl-enrollment-table summarizes the intermediate values for Holt-Winters filtering.1
...
byui_enrollment_ts |>
  as_tibble() |>
  hw_additive_slope_multiplicative_seasonal_rounded("dates", "enrollment_1000", p = 3, predict_periods = 12) |>
...
ggplot(temp2, aes(x = date)) +
    ...
    geom_line(aes(y = estimated_level + estimated_seasonal, color = "Combined", alpha=0.5), linewidth = 1) +
    geom_line(aes(y = forecast, color = "Combined", alpha=0.5), linetype = "dashed", linewidth = 1) +
    coord_cartesian(ylim = c(95,130)) +
    labs(
      x = "Date",
      y = "Value",
