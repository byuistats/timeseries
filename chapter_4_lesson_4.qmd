...
Even though R does not report the parameter for the mean of the process, $\hat \alpha_0 = `r ( mean(dat_ts$x) - tidy(fit_ar) |> select(estimate) |> pull() * mean(dat_ts$x ) ) |> round(3)`$, it is not significantly different from zero. One could argue that we should not use a model that contains the mean and instead focus on a simple fitted model that has only one parameter:
...
It is easy to conceive situations where the mean of an AR model is not zero. 
...
Suppose the mean of the $AR(1)$ process is $\alpha_0 = `r alpha0`$. We will set $\alpha_1 = `r alpha1`$, and $\sigma^2 = `r sigma_sqr`$ for this simulation.
...
$$\alpha_0' = \alpha_0 - \alpha_1 ~ (\alpha_0) = `r alpha0 * (1-alpha1)`$$
...
The time plot below illustrates the change in global 
...
-   Is the $AR(`r tidy(global_ar) |> as_tibble() |> dplyr::select(term) |> tail(1) |> right(1)`)$ 
model stationary?
...
-   THE GLOBAL WARMING TIME SERIES GIVEN IN THE BOOK IS STATIONARY!!!
    -   IT WOULD BE FUN TO EXPLORE THE IDEAS ASSOICATED WITH THAT: 
    -   SMALLER SAMPLE SIZE, ESPECIALLY IN THE AREA WHERE THE ACTION IS...
    -   DIFFERENT CONCLUSIONS BASED ON DIFFERENT DATA...
    -   DATA NOT THE SAME, DIFFERENT WAYS OF ESTIMATING THE SAME THING
    -   LOTS OF FUN IMPLICATIONS

-   Explain how the choice of a model can bias the results.
-   Why is it important to carefully consider our model selection, including factors that might affect the results?
...
temps_forecast <- global_ar %>% forecast(h = "20 years")
temps_forecast |>
...
```{r}
# constant terms
alpha0 <- global_ar %>% coefficients() %>% head(1) %>% dplyr::select(estimate) %>% pull()
# other alphas
alphas <- global_ar %>% coefficients() %>% tail(-1) %>% dplyr::select(estimate) %>% pull()
paste0(
        "0 = 1 ", 
...
Note that the constant term is not statistically signficant. If we ignore this term, we get the fitted model:
...
STUFF_GOES_HERE___STUFF_GOES_HERE___STUFF_GOES_HERE___STUFF_GOES_HERE
