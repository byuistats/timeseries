...
In a qmd file, you can do all sorts of fun things. If you type `<ctrl> <alt> i`, RStudio will insert an R code chunk, where you can run any R code. Once you create a code chunk, you can populate it with any R code. The R code below generates 10 uniform random variables between 0 and 1 and displays them to the user.
...
You can use inline R code as well, meaning you can run R code and output results in the middle of a line of text in your qmd file. For example, the mean of the n=`r reps` simulated x values is: `r mean(df1$x)`.
...
Use the commands `mutate` and `ymd_hm()` to convert the `times` variable into a date-type variable. Then use the `filter` command to focus on the four observations from noon to 3 PM (15:00).

There are many ways to accomplish this but here is one simple example:

```{r message=FALSE}
pulse_df_filtered <- pulse_df |>
  mutate(times = ymd_hm(times)) |> # convert times to date-type data
  filter(hour(times) >= 12 & hour(times) <= 15) # using the hour() function from lubridate we look at just the hour section of the 'times' variable and then filter to values between 12 and 15
```
...
```{r}
# put your code here
pulse_df |> head() # lists the first 6 values - delete this sample command
```
