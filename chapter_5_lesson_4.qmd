...
A dotted line illustrating the simple least squares line is plotted on this figure for reference. This was not used in the model, but it highlights the curvature in the trend.
...
##### Reduced Quadratic Model 1

This model omits sine and cosine terms where $i>3$.
...
reduced_quad_lm1 <- sim_ts |>
  model(reduced_quad1 = TSLM(log(x_t) ~ std_t + I(std_t^2) + 
    sin1 + cos1 + sin2 + cos2 + sin3 + cos3))
reduced_quad_lm1 |>
  tidy() |>
  mutate(sig = p.value < 0.05)
...
reduced_quad_lm2 <- sim_ts |>
  model(reduced_quad2 = TSLM(log(x_t) ~ std_t + I(std_t^2) + 
    sin1 + cos1 + sin2 + cos2))
reduced_quad_lm2 |>
  tidy() |>
  mutate(sig = p.value < 0.05) 
...
reduced_quad_lm3 <- sim_ts |>
  model(reduced_quad3 = TSLM(log(x_t) ~ std_t + I(std_t^2) + 
    sin1 + cos1)) 
reduced_quad_lm3 |>
  tidy() |>
  mutate(sig = p.value < 0.05) 
...
We now fit a linear model to the log-transformed time series.
...
model_combined <- sim_ts |>
  model(
    ...
    full_quad = TSLM(log(x_t) ~ std_t + I(std_t^2) + 
                       sin1 + cos1 + sin2 + cos2 + sin3 + cos3 
                     + sin4 + cos4 + sin5 + cos5 + cos6 ),
    reduced_quad1 = TSLM(log(x_t) ~ std_t + I(std_t^2) + sin1 + cos1 + sin2 + cos2 + sin3 + cos3),
    reduced_quad2 = TSLM(log(x_t) ~ std_t + I(std_t^2) + sin1 + cos1 + sin2 + cos2),
    reduced_quad3 = TSLM(log(x_t) ~ std_t + I(std_t^2) + sin1 + cos1),
...
The quadratic term is not statistically significant in this model. 
...
log_transform_lm <- sim_ts |>
  model(full_cubic = TSLM(log(x_t) ~ std_t +
    sin1 + cos1 + sin2 + cos2 + sin3 + cos3 
    + sin4 + cos4 + sin5 + cos5 + cos6 ))
