...
## Introduction to the course structure and Canvas (40 min)
...
-   Lesson cadence
    -   Read assigned section(s) from the textbook
        -   The course follows the chapter and lesson numbering in the textbook
    -   Reading Journals
        -   Record your learning
        -   Organized by learning outcomes (List each outcome and state important ideas and your questions related to that outcome)
...
-   Benefits of Learning about Time Series
    -   Techniques taught in a linear regression course do not account for the relationship between successive observations
        -   Non-stationary series can show a strong linear relationship purely by chance due to shared trends, giving you a high, but meaningless \$R^2\$ value 
        -   Autocorrelation causes standard errors to be severely underestimated, which leads to overconfident p-values, false significance in t-tests, and wrong confidence intervals
        -   Systematic errors (residuals) compound over time instead of averaging out to zero, making future predictions completely inaccurate
    -   Distinguish yourself from other job applicants
    -   Increased starting salary (approximately $10,000 on average)

## Class Activity: Google Trends (Searches for "Chocolate") (15 min)

Google Trends (<a href="https//trends.google.com">trends.google.com</a>) allows you to download a time series showing the proportional number of searches for a given term. The month with the highest number of searches has a value of 100. The values for the other months are given as a percentage of the peak month's value. The following table illustrates the data, as given by Google Trends.
...
The cleaned version of the data used for this demonstration are available in the file <a href="data/chocolate.csv" download>chocolate.csv</a>. 

If you have used R before, you have probably learned to read data into a data frame. For time series data, we will use a specialized data frame called a tsibble, or a time-series tibble object. This is a data structure in R that handles time series data beautifully. 

<!-- `chocolate_month <- rio::import("https://byuistats.github.io/timeseries/data/chocolate.csv")` -->

In Lesson 3 of Chapter 1, we will practice converting data like this into a time series (tsibble) object. Please run the following code to create a tsibble containing the chocolate data.
...
We will use the tsibble object (which in this case is called *chocolate_month_ts*) to explore this time series. Here is a plot of the time series representing the proportional frequency of searches for the term "chocolate."
...
-   Think about what you know about the reported number of searches in December compared to the following February. The reported relative number of searches for "chocolate" in December `r example_year-1` is `r chocolate_month_ts |> filter(year == example_year - 1, month == 12) |> data.frame() |> dplyr::select(value) |> pull()`. Does it make sense that the reported realative number of searches in February `r example_year` is `r chocolate_month_ts |> filter(year == example_year, month == 2) |> data.frame() |> dplyr::select(value) |> pull()` ? Given the value from December, is the value in the following February independent and completely random?
...
Discuss these vocabulary terms in the context of the Google Trends ("Chocolate") example: 
