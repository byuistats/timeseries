...
<!-- The following was moved to Chapter 1, Lesson 2 -->
...
<!-- The following app illustrates a few realizations of a time series.  -->
...
<!-- -   Use the figure above to explain the following vocabulary terms to your partner: -->
...
## Hands-on Exercise -- Exploring Sample Autocorrelation (45 min)
...
We can compute the **lag 1 autocorrelation** or the **autocorrelation of** $x$ with lag 1 as the quotient $r_1 = \frac{c_1}{c_0}$, where 

$$ c_1 = \sum\limits_{t=1}^{n-k} (x_t-\bar x)(x_{t+k}-\bar x) $$ 

by 

$$ c_0 = \sum\limits_{t=1}^{n} (x_t-\bar x)^2 $$
...
Now, we use $c_0$ to compute $r_1$. Here are two ways we can compute this value:
...
-   Working with your assigned partner, fill in the blanks in the following table to compute $c_2$ and $r_2$.
...
The figure below illustrates the correlations between $x_t$ and $x_{t+3}$.

::: {.callout-tip icon=false title="Check Your Understanding"}

-   Does the value of $r_3 = \dfrac{`r summarized$c_k`}{`r summarized$c_0`} = `r summarized$r_k`$ seem reasonable, given the pattern in this plot?
...
-   Compute $c_4$ and $r_4$ using R, Excel, or hand calculations.
...
$$
  r_2 = \frac{c_2}{c_0} = \frac{`r summarized$c_k |> round(3)`}{`r summarized$c_0 |> round(3)`} = `r summarized$r_k |> round(3)`
$$
...
$$
  r_4 = \frac{c_4}{c_0} = \frac{`r summarized$c_k |> round(3)`}{`r summarized$c_0 |> round(3)`} = `r summarized$r_k |> round(3)`
$$
