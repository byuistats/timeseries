...
#| eval: false
pacman::p_load(
    # packages needed for student-facing code on that page
)
```
...
## Class Activity: Introduction to Autoregressive Moving Average (ARMA) Models (xxx min)
...
reduced5_linear_lm <- weather_df |>
  model(reduced_linear_5  = TSLM(x ~ zTIME + month_index))
