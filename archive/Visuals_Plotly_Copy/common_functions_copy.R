...
display_arima_models_1 <- function(models_ts) {
  # Identify "best" models
  extrema <- models_ts |>
    reframe(
      sigma2 = which(min(sigma2)==sigma2),
  ...
  models_ts %>%
    ...
    mutate(
      ...
      sigma2 = ifelse(round(sigma2, 1) == round(min(sigma2), 1), paste0("<b>", round(sigma2, 1), "</b>"), round(sigma2, 1)),
      log_lik = ifelse(round(log_lik, 1) == round(min(log_lik), 1), paste0("<b>", round(log_lik, 1), "</b>"), round(log_lik, 1)),
    ) %>%
    display_table()
}
...
get_toy_data <- function(n = 10, mu = 0, sigma = 3, rho = 0.99, random_seed = 997) {
  ...
  return( round(MASS::mvrnorm(1, rep(mu,n), sigma^2 * tmp.r),1) )
}
