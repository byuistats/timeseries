...
Here is a plot of the EWMA (in red) and the random component (in black) for a four-hour period:
...
Here is a plot showing the EWMA (in red) for all the data.
...
sum(components(ny_speeds_model1)$remainder^2, na.rm = T)
```
...
```{r}
sum(components(ny_speeds_model0)$remainder^2, na.rm = T)
# 77661.59
```
