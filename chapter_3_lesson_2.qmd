...
Start filling in the table below by calculating the values of $a_t$.
...
-   Compute $e_t$ for the 10 values of $x$ given above. Add these values to the table.
...
The following code can be used to compute the EWMA for the random component of the New York highway speed data in R.

We have already decomposed the model, so we have isolated the estimated values for the random component. This is portion of the tsibble `ny_speeds_decompose`:
...
We execute the EWMA algorithm on the random component using $\alpha = 0.2$.
...
<!-- This will allow R to optimize $\alpha$ by minimizing the $SS1PE$. -->
    ...
    -   What is the value of the $SS1PE$?
-   Choose values of $\alpha$ above and below the one you just found. Can you find a value of $\alpha$ that gives you 
...
alpha_est <- ny_speeds_model0[[1]][[1]][["fit"]][["par"]][["estimate"]][1]

min_ss1pe <- sum(components(ny_speeds_model0)$remainder^2, na.rm = TRUE)
...
Hello `r sum(components(ny_speeds_model0)$remainder^2, na.rm = TRUE) |> round(2)`
