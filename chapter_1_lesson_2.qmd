...
## Comparison of Deterministic and Stochastic Trends in Time Series (10 min)
...
If you haven't already, install the `pacman` package.

We will use `pacman` in place of `library()` to load packages because it reads simpler and downloads packages if you don't have them.
...
Not every package here is used every day, but these are the most common. You can expect to load these every time, then load extra packages as they are needed.
