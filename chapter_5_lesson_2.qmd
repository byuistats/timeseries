...
pacman::p_load(PerformanceAnalytics) # skewness()
```
...
tidy(log_ar) |> select(!.model)
```

```{r}
#| code-summary: "Show the code: Improved month labels simple"
#| code-fold: true
#| output: false
...
tidy(log_ar) |>
  select(!.model) |> 
  mutate(
    term = if_else( # Find season label, replace with month abb using regex
...
```{r}
#| code-summary: "Show the code: Improved month labels readable"
#| code-fold: true

# Helper function to pretty the labels from season(12)
# `term` is a string that looks like: "season(12).season_122"
convert_season_label_to_month <- function(term) {
  # Pull out the end number using regex (ie 2, 3, ..., 12)
  month_num <- as.integer(str_extract(term, "12(\\d+)$", 1))
  
  # Return the month name for that number (ie "Feb" for 2)
  month.abb[month_num]
}
...
tidy(log_ar) |>
  select(!.model) |> 
  mutate(
    # Pretty the month labels with regex and month.abb
    term = if_else(               
      str_detect(term, "season"),          # IF the term label has "season" in it
      convert_season_label_to_month(term), # Make the label readable (function above)
      term                                 # ELSE keep the label as is
    ),
...
::: {.callout-important title="Warning: Interpreting season(12)"}
...
-   Use AR to fit the model, if there is evidence of autocorrelation in the random component.
...
We check for autocorrelation in the random component to determine if using additional fitting is warranted.
...
::: {.callout-caution icon=false title="Applying Autoregression, AR" collapse="false"}

Recall that in [Chapter 5, Lesson 1](https://byuistats.github.io/timeseries/chapter_5_lesson_1.html#FittingRegModelWithPACF), we fitted a linear regression model using the value of the partial autocorrelation function for $k=2$. This helps account for the autocorrelation in the residuals.
...
log_cube_ar <- retail_ts |> 
  model(AR(log(sales_millions) ~ std_t + I(std_t^2) + I(std_t^3) + season(12) + order(2)))

tidy(log_cube_ar) |> select(!.model)
```
