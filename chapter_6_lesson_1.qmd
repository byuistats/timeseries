...
$$
  x_t 
  =(1 + \beta_1 \mathbf{B} + \beta_2 \mathbf{B}^2  + \beta_3 \mathbf{B}^3  + \cdots + \beta_{q-1} \mathbf{B}^{q-1}  + \beta_q \mathbf{B}^{q}) w_t 
  = \phi_q(\mathbf{B}) w_t
$$
...
$$
  x_t 
    = (\text{some}~3^{rd}~\text{degree polynomial in}~\mathbf{B}) w_t
    = \phi_3(\mathbf{B}) w_t
$$
