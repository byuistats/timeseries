...
rexburg_day <- weather_dat %>%
  mutate(date_seq = ymd(dates)) %>%
  mutate(
...
rexburg_daily_ts <- rexburg_day |> 
  dplyr::select(-rexburg_airport_high) |>
  mutate(index = ymd(dates)) |>
  as_tibble() |>
  as_tsibble(index = index)

rexburg_annual_ts <- summarise(index_by(rexburg_daily_ts, year(index)), high_temp = mean(high_temp))
