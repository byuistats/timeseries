...
$$
This model would be appropriate for data where there are $s$ seasons and the value in the season exactly one cycle prior affects the current value. This model is stationary when $\left| (1/\alpha)^{1/s} \right| > 1$.
...
$$
  x_t = x_{t-1} + \alpha x_{t-12} - \alpha x_{t-(12+1)} + w_t
$$
...
which is $ARIMA(0,1,0)(1,0,0)_{12}$.
